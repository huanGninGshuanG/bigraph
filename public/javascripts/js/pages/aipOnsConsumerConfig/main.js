...
var openAipCConfigLayer = function(update, entityId, updatebtn){
	...
	top.layer.open({
		type : 2,
		title : FORM_TITLE_PRE + 'ONS配置管理表单页',
		closeBtn : 1, // 不显示关闭按钮
		shadeClose : false,
		shade : false,
		scrollbar: false,
		...
		area : [ '60%', '50%' ],
		shade : [ 0.3 ],
		content : 'pages/aipOnsConsumerConfig/form.html?isUpdate=' + update + '&entityId=' + entityId + paramStr + '&v=' + version,
		btn : [ '保存', '取消' ],
...
function operateRender(data, type, full) {
	var buttons = [];
	buttons.push('<a onclick="dtOptions.lookup(\'' + data + '\', \'pages/aipOnsConsumerConfig/form.html\',\'ONS配置管理表单页\',60,50)" class="tb_a">查看</a><i class="tb_i">|</i>');
	buttons.push('<a onclick="openAipCConfigLayer(true, \'' + data + '\')" class="tb_a">修改</a><i class="tb_i">|</i>');
	buttons.push('<a onclick="deleteSelf(\'../../ons_consumer/delete.do\', \'' + data + '\', \'aipConsumerConfugGrid\')" class="tb_a">删除</a>');
	return buttons.join(' ');
}
