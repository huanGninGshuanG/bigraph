...
function queryGrid(menuCode) {
	...
	$("#aipDIPComponentGrid").DataTable().fnSearch();
	
}
// 非节点form表单页面

// 打开layer
var openaipDIPComponentLayer = function(update, entityId, updatebtn) {
	...
	top.layer
			.open({
				type : 2,
				title : FORM_TITLE_PRE + '组件管理管理表单页',
				closeBtn : 1, // 不显示关闭按钮
				shadeClose : false,
				shade : false,
				scrollbar : false,
				...
				area : [ '60%', '50%' ],
				shade : [ 0.3 ],
				content : 'pages/aipDIPComponents/form.html?isUpdate='+ update + '&entityId=' + entityId + paramStr + '&v='+ version,
				btn : [ '保存', '取消' ],
				yes : function(index, layero) {
					DxsWebServiceIsRefresh = true;
					var iframeWin = layero.find('iframe')[0]; // 得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
					iframeWin.contentWindow.FormUtils.submitHandler(window.name, "submitAipDataComponentForm","aipDIPComponentForm");
				},                                                                                             
				...
				success : function(layero, index) {
					...
					if (DxsWebServiceIsRefresh)
						$("#aipDIPComponentGrid").DataTable().draw();
				}
			});
}
...
function operateRender(data, type, full) {
	var buttons = [];
	buttons.push('<a onclick="dtOptions.lookup(\'' + data + '\', \'pages/aipDIPComponents/form.html\',\'组件管理表单页\',60,60)" class="tb_a">查看</a><i class="tb_i">|</i>');
	buttons.push('<a onclick="openaipDIPComponentLayer(true, \'' + data + '\')" class="tb_a">修改</a><i class="tb_i">|</i>');
	buttons.push('<a onclick="deleteSelf(\'../../dipcomponent/delete.do\', \'' + data + '\', \'aipDIPComponentGrid\')" class="tb_a">删除</a>');
	return buttons.join(' ');
}
