...
object Match {
  ...
  def merge(a: Set[Match], b: Set[Match]) = {
    // logger.debug("BUG: match::merge(): " + a.size + " with " + b.size)
    var n: Set[Match] = Set()
    n ++= a
    n ++= b
    n
...
class Match(reactionRule: ReactionRule) {
  ...
  var names = Map[Name, Name]()           // rr.name -> agent.name    属于 link mapping
  var parameters = Map[Int, Term]()       // 用来记录site 里的填充物的
  var mapping = Map[Term, Term]()         // agent.term -> rr.term    注意这里映射的方向是反的，从偶图中的部分映射到 反应规则中的redex
  var nodeMap = Map[Node, Node]()         // rr.node -> agent.term

  var reactNodes: Set[String] = Set()     //这里应该是node名不是control名
  var reactNodesMap: Map[String, Set[String]] = Map()   // ctrl.name  -> Set[node.name]
  ...
  var assignValue: Map[Node, Double] = Map() // added by kgq

  //logger.debug("BUG: match::match(): new match created")
...
//    reactNodesMap.clear()
  ...
  def addParam(id: Int, c: Term): Unit = {
    if (c == null) {
      logger.debug("BUG: attempted to add_param for null term in hole " + id)
      sys.exit(1)
    }
    parameters += (id -> c)
  }
  ...
  def addMatch(src: Term, target: Term) = {
    if (src == null || target == null) {
      logger.debug("BUG: attempted to add null match")
      sys.exit(1);
    }
    mapping += (target -> src)
  }
  ...
  def getName(n: Name): Name = {
    if (n == null) {
      null
    } else if (!Bigraph.isFree(n)) {    // isFree 一直为true
      n
    } else if (parent == null) {
      if (names.contains(n)) {
        names(n)
  ...
  def conflict(rns: List[String]): Boolean = {//判断reactNodes:Set[String]与rns这个List中有没有冲突的nodes，冲突就是不可以并发反应
    ...
    rns.map(rn => {
      if (reactNodes.contains(rn)) {
        logger.debug("match " + rule.name + "\tconfilict node:" + rn)
        conflict = true
      }
    })
    return conflict
  ...
  def success: Unit = {
    if (hasFailed) {
      logger.debug("BUG: match::success(): match has already failed!")
      sys.exit(1)
    }
    hasSucceeded = true
    hasFailed = false
  ...
  override def clone: Match = {
    // logger.debug("BUG: match::clone(): new match created")
    ...
    if (m.root != null) {
      // logger.debug("BUG: match::clone(): setting root: " + m.root.toString())
    }
    m
  }

  def fresh(head: Term, rem: List[Term]): Match = {
    logger.debug("BUG: match::fresh(): new match created")
    new Match(rule)
  }
  ...
  override def toString: String = {
    ...
    s += "React Nodes (Set[node.name]):\n"
    reactNodes.foreach(rn => { s += "\t" + rn + " "})

    s += "React Nodes (ctrl.name -> Set[node.name]):\n"
    for ((key, value) <- reactNodesMap) {
      s += "\t" + " -> " + value.toString() + "\n"
    }
    s
  }
