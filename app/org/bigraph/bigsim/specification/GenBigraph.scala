...
object GenBigraph {
  ...
  var automaton = GenTS.genTS(); //lry:获取属性集，以迁移对方式返回
  //由automation转换后的所有Bigraph pair
  ...
  def getstateBigraph(): Map[String, Bigraph]  = {
    ...
    automaton.foreach { x => 
      ...
      println("labelSet.size="+labelSet.size);//lry
      println("labelSet="+labelSet);//lry
      if(labelSet.size == 1) {
        //如果label不是sigma
        if (!isSIGMA(labelSet)) {
          if(x.getSourceState.isInitial()) {
            var sourceStr = x.getSourceState.getLabel;
            //println("sourceStr="+sourceStr);//lry
  ...
  def getBigraphPair(): Unit = {
    ...
    automaton.foreach { x => 
      var labelSet: scala.collection.mutable.Set[IGraphProposition] = x.getLabels;
      println("\n\n\nlabelSet="+labelSet);
      println("labelSet.size="+labelSet.size);
      if(labelSet.size == 1) {
        if (isSIGMA(labelSet)) { //标签为siegma
          ...
          println("source label="+x.getSourceState.getLabel);//lry
          println("source***="+source);//lry
          
          var target = stateBigraph(x.getTargetState.getLabel);
          println("target label="+x.getTargetState.getLabel);//lry
          println("target***="+target);//lry
  ...
  def getAllBigraph():Set[Bigraph] = {
    ...
    bigraphPairList.foreach { x => 
      println("x=="+x);//lry
      println("source"+x.sourceBigraph);//lry
      println("target"+x.targetBigraph);//lry
      res.add(x.sourceBigraph);
      res.add(x.targetBigraph); 
     }
    println("AllBigraph="+res);//lry
    return res;
  }
