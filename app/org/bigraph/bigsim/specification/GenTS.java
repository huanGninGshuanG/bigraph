...
public class GenTS {
	...
	public static ArrayList<ITransition> genTS() {
		...
		//String formula = arr[0];//lry
		String formula = Specification.processSpec().formula();//lry
		System.out.println("formula="+formula);//lry
		String negFormula = "!" + formula; 
		ArrayList<ITransition> res = new ArrayList<ITransition>();
		Collection<ITransition> automaton = LTL2BA4J.formulaToBA(negFormula);//lry:把formula转化为有限状态自动机
		System.out.println(automaton.size());
		for (ITransition t : automaton) {
			System.out.println("Transition---" + t);//lry
			System.out.println(t.getLabels());//lry
