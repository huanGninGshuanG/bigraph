...
//  这个是用来修复Term签名方法的，使用Term的前序字符串的哈希值作为签名， 详见谭朝华学长论文
object TermString {
  ...
  def preOrderString(t: Prefix): String = {
    if ("" == t.node.name) {
      t.node.ctrl.name + getNumber(t.node) + fports(t.node.ports) + "." + preOrderString(t.suffix);
    } else {
      if (GlobalCfg.node)
        t.node.name + ":" + t.node.ctrl.name + getNumber(t.node) + fports(t.node.ports) + "." + preOrderString(t.suffix);
      else
        t.node.ctrl.name + getNumber(t.node) + fports(t.node.ports) + "." + preOrderString(t.suffix);
    }
  }
  ...
  def getInnerName(n: Name): String = {
    var ret: String = ""
    if (!n.innerNameList.isEmpty)
      ret += "(in: " + n.innerNameList.mkString(",") + ")"
    ret
  }
