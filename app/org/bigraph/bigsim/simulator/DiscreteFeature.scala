      ...
      while (reactList.size > 0) {//当前可并发反应的规则
        ...
        if (matches != null) {
          ...
          matches.map(m => {
            ...
            println("probability --> " + m.rule.hasProbability)
            if(m.rule.hasProbability){
              probabilityResult = randomProbability(m.rule.probability)
              println("probabilityResult --> " + probabilityResult)
            }
...
//        }
  ...
  def addMatch(): Boolean = { //此方法加到simQueue
    ...
    // 上来先进行四项检查：steps，Tracking，Binding，Sorting
    if (steps >= GlobalCfg.maxSteps) {
     ...
     if (GlobalCfg.checkBinding) {//BINDING一旦违反就停止
      var bindSet: Set[Binding] = v.bigraph.bindings    // 这里的Binding是Binding Constraint，这里的v是从工作队列中取出来的节点，
      var res: Boolean = true         // 对当前的这个偶图，检测它是不是满足所有的binding constraint
        bindSet.foreach { x => {
          res = res && x.check(v)
        }
      }
      if(!res){     // 如果最后的结果是false，那么就表示当前的状态不满足binding限制。
        v.terminal=true
