...
import cern.jet.random.{Exponential, Uniform}
import cern.jet.random.engine.RandomEngine
import org.bigraph.bigsim.BRS.{Graph, Vertex, _}
...
class StochasticSimulator(b: Bigraph) extends Simulator {
  ...
  def simulate: Unit = {
    ...
    System.out.println("=========================================================")
    System.out.println("=============StochasticSimulator=========================")
    System.out.println("=========================================================")

    println("rules:    "  + b.rules)
    /**
    ...
    while (systemActivity > 0) {//systemActivity=0系统模拟结束
      ...
      println("reaction rules: " + simRules)
    ...
    println("=================simPath==================")
    println(simPath)
    println("simPath: " + simRules)
    paths = dump(simPath, simRules.size)
    println("===============paths===============")
    println("path  " + paths)
    StochasticSimulator.matchGC;
  }

  def dump(simPath: Queue[Term], simRules: Int): String = {//打印到path文件
    var out: String = "";
    dumpDotForward("")
