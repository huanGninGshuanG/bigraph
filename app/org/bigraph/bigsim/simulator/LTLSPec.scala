...
class LTLSpec(ltlSpec: List[String], ltlProp: Map[String, Tuple2[String, String]]) {            // 在这个小括号里定义的变量，会自动成为类的成员，不需要在类的内部增加新的定义
  ...
  def getLTLFormula(): Formula = {
    this._formula
  }

  def getLTLFormula(ltlSpec: List[String], ltlProp: Map[String, Tuple2[String, String]]): Formula = {
    ...
    def oneOp() = { // 一次操作，先从操作符栈中取出来最上面的，然后根据情况，选择从公式栈中取出，构造当前的公式，然后再放入公式栈
      ...
      top_o match {
        ...
        case "R" => {
          ...
          val top_f2 = formula.pop()
          if (debug) print("\t [U] top_f1: " + top_f1 + " top_f2: " + top_f2)
          val currentF = new LTLOperatorR(top_f2, top_f1)
          formula.push(currentF)
          if (debug) println("\t [A] formula.push " + formula)
          //operand.push("(")
        }
...
object TestlTLFunc {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Output:" , ltlParser.getLTLFormula)
  }
}
