...
class DiscreteEventSimulator(b: Bigraph) extends Simulator {
  ...
  def simulate: Unit = {
    if (b == null || b.root == null) {
      logger.debug("DiscreteEvent::simulate(): null");
      return;
    } else {
      if (GlobalCfg.printMode) {
        logger.debug("N_" + Math.abs(v.hash)+":{}", v.bigraph.root);
        logger.debug(v.variables)
      }
  ...
  def backwordStep(): Boolean = {
    ...
    if (steps >= GlobalCfg.maxSteps) {
      logger.debug("simulate::step Interrupted!  Reached maximum steps: " + GlobalCfg.maxSteps);
      report();
      return false;
    }
    // 待检测模型队列为空
    if (forwardPath.size == 0) {
      logger.debug("sim::step Complete!");
      report();
      DiscreteEventSimulator.matchGC;
      return false;
    }
    ...
    if (matches.size > 0) {
      ...
      matches.map(it => {
        //就是Match的reactNodes和本模拟器定义的reactNodes比较看有没有重复的node，有就认为是冲突，相当于TimeSlicingSimulator调用的Match的conflict方法，逻辑一样
        var conflict = false
        ...
        if (conflict) {
          //冲突，模拟器的reactNodes集合里面已经有该node，故删去

          //从matches: Set[Match]中删除it
          matches -= it
        } else if (!it.rule.random) {
          //不冲突且该RR不是随机反应，那就应该必定发生反应，加入到待反应集合simRRMap中，必然事件，必定发生
          var key = (GlobalCfg.SysClk + it.rule.getRRIncr).toString + "_" + scala.util.Random.nextInt(100000)
          while (simRRMap.contains(key)) {
            //指导key随机出一个simRRMap中没有的才退出循环，key的前半部分是当前系统时间加上反应花费时间，即反应完成的时间点，这部分是一样的，后面的随机数可以不一样
            key = (GlobalCfg.SysClk + it.rule.getRRIncr).toString + "_" + scala.util.Random.nextInt(100000)
          }
          //(key,it)放入simRRMap中
          simRRMap += key -> it
          //合并两个set，把match的reactNodes放入simulator的forwardReactNodes中
      ...
      if (matches.size > 0) {
        //这时候的matches是筛选过后的Set[Match],只剩下不冲突但是随机反应的规则
        var randIndex = scala.util.Random.nextInt(matches.size)
        //在这个集合中随机一个位置应用

        //先把Set转为List然后取下标为randIndex的Match元素，这所有的随机集合中就选一条出来反应，随机事件，随机一个来发生
    ...
    if (curBigraph != null && curRR != null) {
      ...
      if (GlobalCfg.printMode) {
        logger.debug("N_"+Math.abs(nv.hash) + ":{}", nv.bigraph.root);
        logger.debug(nv.variables)
      }
    }
    matches.clear();
    DiscreteEventSimulator.matchGC;
