...
class TransitionSystem(b: Bigraph) {
  ...
  def step(): Boolean = {
    ...
    if (!GlobalCfg.useCSP) {
      ...
      val pair = b.findAllMatchByCSP()
      pair.forEach(p => {
        val big = p._1
        val rr = p._2
        if (big.root == null) big.structToTerm()
        var nv: Vertex = new Vertex(big, v, rr)
        if (g.lut.contains(nv.hash)) {
          nv = g.lut(nv.hash)
          nv.addParents(v)
        } else {
...
class BuildKripkeStructure(trans: TransitionSystem) {
  ...
  def buildKripke(sig: Signature): KripkeStructure = {
    if (this.build)
      return this.kripkeStruct
    buildPropMap(sig) // 先构造原子命题映射： String -> ReactionRule
    buildInitialState
    buildState
    buildTransition
  ...
  def buildPropMap(sig: Signature) = {
    ...
    for ((k, v) <- propStr) { // k是 prop的名字，v是prop的项语言描述
      ...
      val rr = new ReactionRule(k, propRedex, propReact, propExp, sig) // 使用一个原子命题作为反应物构造一个反应规则
